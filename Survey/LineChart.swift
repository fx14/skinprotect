...
public struct Axis {
    public var visible: Bool = true
    // #607d8b
    public var color: UIColor = UtilityClass.uiColorFromHex(0xfc6c5f)// UIColor(red: 96/255.0, green: 125/255.0, blue: 139/255.0, alpha: 1)
...
public struct MyDot {
    // personalized dot details
    public var label: String = ""
    public var age: CGFloat = 0
    public var average: CGFloat = 0
    public var dotColor: UIColor = UIColor()
}
...
public class LineChart: UIView {
    ...
    public var colors: [UIColor] = Singleton.sharedInstance.colors
    ...
    * Draw small dot
    */
    private func drawMyDots() {
        
        for myCurrentDot in myDots {
            let d = dots.outerRadius/2
            let k = x.axis.inset
            let t = self.x.scale(CGFloat(myCurrentDot.age))
            var xValue = self.x.scale(CGFloat(myCurrentDot.age)) + x.axis.inset - dots.outerRadius/2
            var yValue = self.bounds.height - self.y.scale(myCurrentDot.average) - y.axis.inset - dots.outerRadius/2
            var dotLayers: [DotCALayer] = []
            
            // draw custom layer with another layer in the center
            var dotLayer = DotCALayer()
            dotLayer.dotInnerColor = myCurrentDot.dotColor
            ...
            // Dot label
            // TODO: width and heigt
            var label = UILabel(frame: CGRect(x: xValue +  dots.outerRadius, y: yValue - 10, width: 100, height: 20))
            label.font = UIFont.preferredFontForTextStyle(UIFontTextStyleCaption2)
            label.textAlignment = .Left
            label.text = myCurrentDot.label
            label.textColor = myCurrentDot.dotColor
    ...
    */
    private func drawLine(lineIndex: Int) {
        ...
        let xPos = w - w / 1.2
        if lineIndex == 0 {
            label.frame = CGRectMake(xPos, yValue-10, 100, 20)
            label.text = NSLocalizedString("male", comment: "Male")
        } else {
            label.frame = CGRectMake(xPos + 40, yValue-10, 100, 20)
            label.text = NSLocalizedString("female", comment: "Female")
        }
        label.font = UIFont.preferredFontForTextStyle(UIFontTextStyleCaption2)
        label.textAlignment = .Left
        label.textColor = colors[lineIndex]
        self.addSubview(label)
    ...
    */
    private func drawAreaBeneathLineChart(lineIndex: Int) {
        ...
        colors[lineIndex].colorWithAlphaComponent(0.1).setFill()
        // move to origin
    ...
    */
    private func drawXLabels() {
        ...
        for (index, value) in enumerate(xAxisData) {
            ...
            label.font = UIFont(name: "HelveticaNeue-UltraLight", size: 9) //UIFont.preferredFontForTextStyle(UIFontTextStyleFootnote)
            label.textAlignment = .Center
            
            let xTotalValues = xAxisData.count
            let labelsCount = x.labels.values.count
            
            // draw only some points not all
            if index == 0 {
                text = "0"
                label.text = text
                self.addSubview(label)
            } else if index <= 8 {
                text = String(x.labels.values[index-1])
                label.text = text
                self.addSubview(label)
            } else if index == 9 {
                text = "age"
                label.font = UIFont.preferredFontForTextStyle(UIFontTextStyleCaption2)
                label.textColor = x.axis.color
                label.frame = CGRectMake(xValue, y, width + 10, x.axis.inset)
                label.text = text
                self.addSubview(label)
            }
    ...
    */
    private func drawYLabels() {
        ...
        var (start, stop, step) = self.y.ticks
        for var i: CGFloat = start; i <= stop; i += step {
            let t = self.y.scale(i)
            let l = (y.axis.inset * 1.5)
            yValue = self.bounds.height - t - l
            
            if stop - i < step { // If its the last value, put the y label
                var label = UILabel(frame: CGRect(x: -4, y: yValue - 10, width: y.axis.inset + 100, height: y.axis.inset))
                label.font = UIFont.preferredFontForTextStyle(UIFontTextStyleCaption2)
                label.textAlignment = .Left
                label.text = "kg"
                label.textColor = x.axis.color
                self.addSubview(label)
            } else if Int(round(i)) % 10 == 0 { // draw every 10 the label
                var label = UILabel(frame: CGRect(x: -4, y: yValue + 5, width: y.axis.inset + 5, height: y.axis.inset))
                label.font = UIFont(name: "HelveticaNeue-UltraLight", size: 9) // UIFont.preferredFontForTextStyle(UIFontTextStyleCaption2)
                label.textAlignment = .Center
    ...
    */
    ...
    /**
    * Add dot to line chart
    */
    public func addDot(mydot: MyDot) {
        myDots.append(mydot)
    }
